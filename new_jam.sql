...
SELECT
	...
	, po.product_name
	, i.qty
	, SUM(pr.price * i.qty) as total
FROM t_jam_orders o
JOIN t_jam_invoices i ON o.invoice_id = i.invoice_id
JOIN t_jam_products po ON i.product_id = po.product_id
JOIN t_jam_prices pr ON i.price_id = pr.price_id
;